...
export const initMultipartUpload = async ({ fileName, mimeType, fileSize }: MultipartInitInput): Promise<MultipartInitResult> => {
...
export const getMultipartPartUploadUrl = async ({ key, uploadId, partNumber }: MultipartPartUrlInput): Promise<MultipartPartUrlResult> => {
...
export const getMultipartThumbnailUploadUrl = async ({ mimeType }: MultipartThumbnailUrlInput): Promise<MultipartThumbnailUrlResult> => {
...
export const completeMultipartUpload = async ({ key, uploadId, parts, mimeType, fileSize, type, thumbnailKey }: MultipartCompleteInput): Promise<IMedia> => {
...
export const abortMultipartUpload = async ({ key, uploadId }: MultipartAbortInput): Promise<void> => {
    await s3Client.send(
            ...
            try {
                ...
                if (SHOULD_COMPRESS) {
                    ...
                    await new Promise((resolve, reject) => {
                        ffmpeg(tempVideoPath)
                            .videoCodec('libx264')
                            .outputOptions(['-preset ultrafast', '-crf 30', '-movflags +faststart', '-threads 0', '-vf scale=854:-2', '-r 24'])
                            .toFormat('mp4')
                            .on('end', resolve)
                            .on('error', reject)
                            .save(compressedVideoPath);
                    ...
                    logger.info(`Compression time: ${(Date.now() - compressStart) / 1000}s`);
